<template>
  <div v-if="visible" class="space-y-1">
    ...
    <div class="flex flex-wrap items-center gap-1.5">
      ...
      <OpenAIReferralCell :account="props.account" />
    </div>
    ...
    <div v-if="autoResetState" class="flex flex-wrap items-center gap-1 text-[10px]" data-testid="auto-reset-credit-state">
      <span class="inline-flex items-center rounded px-1.5 py-0.5 font-medium" :class="autoResetStateClass">
        {{ autoResetStateLabel }}
        <span v-if="autoResetState.trigger_window" class="ml-1 tabular-nums">{{ autoResetState.trigger_window }}</span>
      </span>
      ...
      <span v-if="autoResetState.error_code" class="max-w-full truncate text-red-600 dark:text-red-400" :title="autoResetState.error_code">
...
const autoResetStateLabel = computed(() => {
  ...
  const keyByStatus: Record<string, string> = {
    checking: 'checking', available: 'available', resetting: 'resetting', success: 'success', no_credit: 'noCredit', failed: 'failed'
  }
  return t(`admin.accounts.openaiQuotaReset.autoStatus.${keyByStatus[status]}`)
})
const autoResetStateClass = computed(() => {
  switch (autoResetState.value?.status) {
    case 'available': return 'bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300'
    ...
    case 'resetting': return 'bg-orange-50 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300'
    default: return 'bg-gray-100 text-gray-600 dark:bg-dark-800 dark:text-gray-300'
  }
})
