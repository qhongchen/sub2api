  ...
  >
    ...
    >
      ...
      >
        <div class="space-y-3">
          <div>
            <label for="grok-media-eligibility-mode" class="input-label mb-0">{{ t('admin.accounts.grokMediaEligibility.title') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
          ...
          <select
            ...
            :disabled="grokMediaEligibilityLoading || submitting"
          >
          ...
          <p v-else-if="grokMediaEligibilityError" class="text-xs text-red-600 dark:text-red-400" role="alert">
            {{ grokMediaEligibilityError }}
          </p>
          <div v-else-if="grokMediaEligibilityState" class="text-xs text-gray-600 dark:text-gray-300">
          ...
          <p v-if="grokMediaEligibilityMode === 'enabled'" class="text-xs text-amber-700 dark:text-amber-400">
            <Icon name="exclamationTriangle" size="sm" class="mr-1 inline" :stroke-width="2" />
      ...
      <!-- OpenAI per-account first-output timeout -->
      <div
        v-if="account?.platform === 'openai' && (account?.type === 'oauth' || account?.type === 'setup-token' || account?.type === 'apikey')"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div class="min-w-0 flex-1">
            <label class="input-label mb-0">{{ t('admin.accounts.openai.firstOutputTimeout') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.openai.firstOutputTimeoutDesc') }}
            </p>
          </div>
          <div class="flex w-full flex-col gap-2 sm:w-72">
            <Select
              v-model="openAIFirstOutputTimeoutMode"
              data-testid="edit-openai-first-output-timeout-mode"
              :options="openAIFirstOutputTimeoutModeOptions"
            />
            <input
              v-if="openAIFirstOutputTimeoutMode === 'custom'"
              v-model.number="openAIFirstOutputTimeoutSeconds"
              type="number"
              min="30"
              max="1800"
              step="1"
              class="input"
              data-testid="edit-openai-first-output-timeout-seconds"
              :placeholder="t('admin.accounts.openai.firstOutputTimeoutSecondsPlaceholder')"
            />
          </div>
        </div>
      </div>
      ...
      >
        <div
          v-if="account?.type === 'oauth' && !isSparkShadow"
          class="space-y-4 border-b border-gray-200 pb-4 dark:border-dark-600"
...
type OpenAIFirstOutputTimeoutMode = 'inherit' | 'custom' | 'disabled'
const openAIFirstOutputTimeoutMode = ref<OpenAIFirstOutputTimeoutMode>('inherit')
const openAIFirstOutputTimeoutSeconds = ref<number | null>(120)
...
const openAIFirstOutputTimeoutModeOptions = computed(() => [
  { value: 'inherit' as OpenAIFirstOutputTimeoutMode, label: t('admin.accounts.openai.firstOutputTimeoutInherit') },
  { value: 'custom' as OpenAIFirstOutputTimeoutMode, label: t('admin.accounts.openai.firstOutputTimeoutCustom') },
  { value: 'disabled' as OpenAIFirstOutputTimeoutMode, label: t('admin.accounts.openai.firstOutputTimeoutDisabled') }
])
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  allowOverages.value = false
	...
	autoResetCredit5hThreshold.value = typeof extra?.auto_reset_credit_5h_threshold === 'number' ? extra.auto_reset_credit_5h_threshold * 100 : 100
	autoResetCredit7dThreshold.value = typeof extra?.auto_reset_credit_7d_threshold === 'number' ? extra.auto_reset_credit_7d_threshold * 100 : 100
  ...
  openAIFirstOutputTimeoutMode.value = 'inherit'
  openAIFirstOutputTimeoutSeconds.value = 120
  editPlanType.value = ''
  ...
  if (newAccount.platform === 'openai' && (newAccount.type === 'oauth' || newAccount.type === 'setup-token' || newAccount.type === 'apikey')) {
    ...
    const firstOutputTimeout = extra?.openai_first_output_timeout_seconds
    if (typeof firstOutputTimeout === 'number' && Number.isInteger(firstOutputTimeout)) {
      if (firstOutputTimeout === 0) {
        openAIFirstOutputTimeoutMode.value = 'disabled'
      } else if (firstOutputTimeout >= 30 && firstOutputTimeout <= 1800) {
        openAIFirstOutputTimeoutMode.value = 'custom'
        openAIFirstOutputTimeoutSeconds.value = firstOutputTimeout
      }
    }
  ...
  if (newAccount.type === 'apikey' && newAccount.credentials) {
    ...
    if (newAccount.platform === 'kimi' || newAccount.platform === 'zhipu' || newAccount.platform === 'deepseek') {
      ...
      const storedAnthropicBaseUrl = typeof storedBaseUrls.anthropic === 'string' ? storedBaseUrls.anthropic.trim() : ''
      const storedResponsesBaseUrl = typeof storedBaseUrls.responses === 'string' ? storedBaseUrls.responses.trim() : ''
      ...
      const storedLegacyBaseUrl = legacyProtocol === 'anthropic'
        ...
        : legacyProtocol === 'responses'
          ...
          : storedChatBaseUrl
      if (legacyBaseUrl && !storedLegacyBaseUrl) nextAdaptiveBaseUrls[legacyProtocol] = legacyBaseUrl
...
const persistGrokMediaEligibility = async (
  ...
  mode: GrokMediaEligibilityMode | null
): Promise<Account> => {
  if (mode === null) return updatedAccount

  const requestVersion = grokMediaEligibilityRequestVersion
  let state: GrokMediaEligibilityState
  try {
    state = await adminAPI.accounts.updateGrokMediaEligibility(accountID, mode)
  } catch {
    appStore.showError(t('admin.accounts.grokMediaEligibility.partialSave'))
    try {
      state = await adminAPI.accounts.getGrokMediaEligibility(accountID)
    } catch {
      return updatedAccount
    }
  }
  if (requestVersion === grokMediaEligibilityRequestVersion && props.account?.id === accountID) {
    grokMediaEligibilityState.value = state
    grokMediaEligibilityInitialMode.value = state.mode
  }
...
const submitUpdateAccount = async (accountID: number, updatePayload: Record<string, unknown>) => {
  ...
  const mediaEligibilityMode = isGrokOAuthAccount.value &&
  ...
  try {
    ...
    updatedAccount = await persistGrokMediaEligibility(accountID, updatedAccount, mediaEligibilityMode)
...
const handleSubmit = async () => {
  ...
  if (
    ...
    openAIFirstOutputTimeoutMode.value === 'custom' &&
    (!Number.isInteger(openAIFirstOutputTimeoutSeconds.value) ||
      (openAIFirstOutputTimeoutSeconds.value ?? 0) < 30 ||
      (openAIFirstOutputTimeoutSeconds.value ?? 0) > 1800)
  ) {
    appStore.showError(t('admin.accounts.openai.firstOutputTimeoutInvalid'))
    return
  }
  ...
  try {
    ...
    if (props.account.platform === 'openai' && (props.account.type === 'oauth' || props.account.type === 'setup-token' || props.account.type === 'apikey')) {
      ...
      if (openAIFirstOutputTimeoutMode.value === 'custom') {
        newExtra.openai_first_output_timeout_seconds = openAIFirstOutputTimeoutSeconds.value
      } else if (openAIFirstOutputTimeoutMode.value === 'disabled') {
        newExtra.openai_first_output_timeout_seconds = 0
      } else {
        newExtra.openai_first_output_timeout_seconds = null
      }
