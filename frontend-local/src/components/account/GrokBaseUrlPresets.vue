...
const emit = defineEmits<{ (event: 'select', url: string): void }>()
const { t } = useI18n()

const presetLabel = (preset: GrokBaseUrlPreset) => {
  if (preset.label) return preset.label
  const key = `admin.accounts.grokCustomBaseUrl.presets.${preset.labelKey}`
  const translated = t(key)
  return translated === key ? (preset.labelKey === 'official' ? 'Official API' : 'Grok Build CLI') : translated
}
