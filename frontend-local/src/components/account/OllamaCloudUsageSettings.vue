  ...
  >
    <div class="flex items-start justify-between gap-4">
      ...
      >
        {{
          state.configured
            ? t('admin.accounts.ollamaCloud.configured')
            : t('admin.accounts.ollamaCloud.notConfigured')
        }}
      </span>
    </div>
    ...
    <template v-else>
      <div
        v-if="!state.encryption_key_configured"
        class="rounded border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:border-amber-800/50 dark:bg-amber-900/20 dark:text-amber-200"
      >
        {{ t('admin.accounts.ollamaCloud.encryptionKeyRequired') }}
      </div>

      <div v-if="snapshot" class="border-y border-gray-100 py-3 dark:border-dark-700">
        <div
          class="grid grid-cols-[minmax(4rem,auto)_minmax(0,1fr)] gap-x-3 gap-y-1.5 text-xs"
        >
      ...
      <div>
        <label class="input-label" for="ollama-cloud-session">
          {{ t('admin.accounts.ollamaCloud.sessionLabel') }}
        </label>
        <textarea
