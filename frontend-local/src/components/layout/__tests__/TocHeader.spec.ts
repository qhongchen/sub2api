...
describe('TocHeader admin top navigation', () => {
  ...
  it('moves personal admin links into the user dropdown and keeps user management under users and access', () => {
    ...
    expect(secondaryNavBlock).not.toContain("'/purchase'")
    expect(secondaryNavBlock).not.toContain("'/orders'")
    expect(secondaryNavBlock).not.toContain("'/redeem'")
    expect(secondaryNavBlock).not.toContain("'/affiliate'")
