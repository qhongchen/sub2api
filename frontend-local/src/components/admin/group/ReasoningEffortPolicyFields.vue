    ...
    <div class="border-t border-gray-200 pt-4 dark:border-dark-600">
      <div class="mb-3 flex items-center justify-between gap-3">
        ...
        <button
          ...
          :class="!canAddMapping && 'cursor-not-allowed opacity-50'"
          :disabled="!canAddMapping"
          :title="!canAddMapping ? t('admin.groups.form.reasoningEffortMappingLimit', { count: MAX_REASONING_EFFORT_MAPPINGS }) : undefined"
          @click="addMapping"
        >
      ...
      <p
        ...
        :role="mappingLimitExceeded ? 'alert' : undefined"
      >
        {{ t('admin.groups.form.reasoningEffortMappingLimit', { count: MAX_REASONING_EFFORT_MAPPINGS }) }}
      </p>
...
const reasoningEffortOptions = computed(() => reasoningEffortOptionsForPlatform(props.platform))
const validationErrors = computed(() => validateReasoningEffortMappings(props.mappings, props.platform))
const canAddMapping = computed(() => props.mappings.length < MAX_REASONING_EFFORT_MAPPINGS)
const mappingLimitExceeded = computed(() => props.mappings.length > MAX_REASONING_EFFORT_MAPPINGS)

const asString = (value: string | number | boolean | null): string => value == null ? '' : String(value)
...
const updateMapping = (
  ...
  emit(
    'update:mappings',
    props.mappings.map((row) => row.id === id ? { ...row, [field]: asString(value) } : row),
  )
}

const addMapping = () => {
  if (!canAddMapping.value) return
  emit('update:mappings', [...props.mappings, createReasoningEffortMappingRow()])
}

const removeMapping = (id: string) => {
  emit('update:mappings', props.mappings.filter((row) => row.id !== id))
}

const mappingErrorText = (code: ReasoningEffortMappingErrorCode | undefined): string =>
  code ? t(`admin.groups.form.${code}`) : ''

const validate = (): boolean => {
  showValidation.value = true
  return !mappingLimitExceeded.value && Object.keys(validationErrors.value).length === 0
}
