<template>
  <section class="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-[1fr_1fr_1fr_300px]">
    <div class="cch-panel-card flex min-h-[280px] flex-col p-5">
      ...
      <div class="flex flex-1 flex-col gap-2">
        ...
        <button
          ...
          :key="entry.id"
          type="button"
          class="group flex items-center gap-3 rounded-lg p-2 text-left transition-colors hover:bg-gray-100/70 dark:hover:bg-white/[0.04]"
        ...
        >
          ...
          <span class="min-w-0 flex-1">
            <span class="mb-1.5 flex items-center justify-between gap-3">
              <span class="truncate text-sm font-semibold text-gray-950 dark:text-white">{{ entry.name }}</span>
              <span class="font-mono text-sm font-semibold text-gray-950 dark:text-white">${{ formatCost(entry.cost) }}</span>
            </span>
            ...
            <span class="mt-1 flex items-center justify-between text-[10px] text-gray-500 dark:text-dark-400">
              <span>{{ formatNumber(entry.requests) }} {{ t('admin.dashboard.requests') }}</span>
              <span>{{ formatTokens(entry.tokens) }} {{ t('admin.dashboard.tokens') }}</span>
            </span>
          </span>
        </button>
    ...
    <div class="cch-panel-card flex min-h-[280px] flex-col overflow-hidden bg-slate-50 p-0 dark:border-white/[0.06] dark:bg-[#0a0a0c]">
      <div class="flex items-center justify-between border-b border-slate-200 bg-slate-100/70 p-4 dark:border-white/[0.06] dark:bg-white/[0.02]">
        <div class="flex items-center gap-2">
          ...
          <span class="font-mono text-xs font-semibold text-slate-700 dark:text-dark-100">
      ...
      <div class="flex-1 overflow-y-auto p-3 font-mono text-xs">
        ...
        <div v-else-if="activeSessions.length" class="space-y-1">
          <button
            ...
            :key="item.id"
            type="button"
            class="group flex w-full items-center gap-2 rounded-md p-2 text-left transition-colors hover:bg-slate-100 dark:hover:bg-white/[0.05]"
            @click="emit('viewAccounts')"
          >
            ...
            <span class="max-w-[72px] flex-shrink-0 truncate text-slate-500 dark:text-dark-400">#{{ item.shortId }}</span>
            <span class="min-w-0 flex-1 truncate font-medium text-blue-600 dark:text-blue-300">{{ item.name }}</span>
            <span class="text-right font-bold text-emerald-600 dark:text-emerald-300">
