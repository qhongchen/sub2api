<template>
  <div class="flex items-center gap-1">
    ...
    >
      ...
      <span class="text-xs">
        {{ duplicating ? t('admin.channelMonitor.duplicating', '复制中') : t('admin.channelMonitor.duplicate', '复制') }}
      </span>
    </button>
    <button
      @click="$emit('edit', row)"
...
const duplicateTitle = computed(() => {
  if (props.row.api_key_decrypt_failed) {
    return t('admin.channelMonitor.duplicateKeyUnavailable', '密钥无法解密，请先编辑并重新保存')
  }
  if (props.duplicating) return t('admin.channelMonitor.duplicating', '复制中')
  return t('admin.channelMonitor.duplicate', '复制')
})
</script>
