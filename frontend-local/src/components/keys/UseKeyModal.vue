...
$env:CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1`
  ...
  return [
    { path, content },
    { path: vscodeSettingsPath, content: vscodeContent, hint: t('keys.useKeyModal.claudeSettingsHint') }
  ]
}
...
function generateGrokClaudeFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  const path = isUnix ? 'Terminal' : activeTab.value === 'cmd' ? 'Command Prompt' : 'PowerShell'
  const content = Object.entries(environment)
    .map(([name, value]) => isUnix ? `export ${name}="${value}"` : activeTab.value === 'cmd' ? `set ${name}=${value}` : `$env:${name}="${value}"`)
    .join('\n')
  const settingsPath = isUnix ? '~/.claude/settings.json' : '%USERPROFILE%\\.claude\\settings.json'
