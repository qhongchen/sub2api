<template>
  <AppLayout>
    <TablePageLayout>
          ...
          <div class="flex flex-col gap-4 p-5 lg:flex-row lg:items-start lg:justify-between">
            <div class="min-w-0 space-y-2">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span class="rounded-full border border-gray-200 bg-white px-2.5 py-1 text-xs font-medium text-gray-500 dark:border-dark-700 dark:bg-dark-800 dark:text-dark-300">
                  {{ channels.length }} {{ t('common.visible') }}
                </span>
                <span class="rounded-full border border-primary-200 bg-primary-50 px-2.5 py-1 text-xs font-medium text-primary-700 dark:border-primary-500/20 dark:bg-primary-500/10 dark:text-primary-200">
                  {{ activeChannelCount }} {{ t('admin.channels.statusActive', 'Active') }}
                </span>
              </div>
      ...
      <template #filters>
        <div class="cch-toolbar-card flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
          <!-- Left: Search + Filters -->
...
const activeChannelCount = computed(() => channels.value.filter(channel => channel.status === 'active').length)
  ...
  for (const section of form.platforms.filter(s => s.enabled)) {
    for (const entry of section.model_pricing) {
      ...
      if ((entry.billing_mode === 'per_request' || entry.billing_mode === 'image' || entry.billing_mode === 'video') &&
        ...
        appStore.showError(t('admin.channels.form.perRequestPriceRequired', '按次/图片/视频计费模式必须设置默认价格或至少一个计费层级'))
        return
      }
    }
  }
