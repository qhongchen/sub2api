  ...
  <AppLayout>
    <div class="space-y-6">
      ...
      <template v-else>
        ...
        <p v-if="status?.enabled && status.risk_control_enabled && status.mode !== 'off' && (status.pre_block_api_key_available_count ?? 1) === 0" class="text-sm text-amber-700 dark:text-amber-300" role="status">
...
const engineFields = ['base_url', 'model', 'proxy_id', 'api_keys_text', 'api_key_configured', 'api_key_masked', 'api_key_count', 'api_key_masks', 'api_key_statuses', 'api_keys_mode', 'clear_api_key', 'timeout_ms', 'retry_count'] as const
