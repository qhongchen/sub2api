          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-2">
              ...
              >
                ...
                <span class="admin-row-action-label">{{ t("admin.groups.compositeRoutes.action") }}</span>
              </button>
              <button
                type="button"
            ...
            <div class="max-h-64 space-y-2 overflow-y-auto p-2">
              ...
              >
                ...
                <button
                  type="button"
                  :disabled="index === 0"
                  :title="t('admin.groups.modelAllowlist.moveUp')"
                  class="shrink-0 rounded p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-700 disabled:opacity-40 dark:hover:bg-dark-600 dark:hover:text-gray-200"
                ...
                <button
                  ...
                  :title="t('admin.groups.modelAllowlist.moveDown')"
                  class="shrink-0 rounded p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-700 disabled:opacity-40 dark:hover:bg-dark-600 dark:hover:text-gray-200"
                  @click="moveCreateModelAllowlistItem(index, index + 1)"
                >
            ...
            <div class="border-t border-gray-200 px-3 py-2 dark:border-dark-600">
              <div class="flex items-center gap-2">
                <input
                  ...
                  :aria-label="t('admin.groups.modelAllowlist.customPlaceholder')"
                  :aria-invalid="!!createAllowlistCustomErrorKey"
                  :aria-describedby="createAllowlistCustomErrorKey ? 'create-model-allowlist-error' : undefined"
                ...
                <button
                  type="button"
                  class="inline-flex shrink-0 items-center gap-1.5 whitespace-nowrap rounded bg-primary-600 px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-primary-700"
              ...
              <p
                ...
                id="create-model-allowlist-error"
                role="alert"
            ...
            <div class="border-t border-gray-200 px-3 py-2 dark:border-dark-600">
              <div class="flex items-center gap-2">
                <input
                  ...
                  :placeholder="t('admin.groups.modelAllowlist.customPlaceholder')"
                  :aria-label="t('admin.groups.modelAllowlist.customPlaceholder')"
                  :aria-invalid="!!editAllowlistCustomErrorKey"
                  :aria-describedby="editAllowlistCustomErrorKey ? 'edit-model-allowlist-error' : undefined"
                  class="min-w-0 flex-1 rounded border border-gray-300 bg-white px-2 py-1.5 text-sm text-gray-700 focus:border-primary-500 focus:outline-none dark:border-dark-500 dark:bg-dark-700 dark:text-gray-200"
    ...
    <BaseDialog
      ...
      :title="
        compositeRoutesGroup
          ? t('admin.groups.compositeRoutes.titleWithGroup', { name: compositeRoutesGroup.name })
    ...
    >
      <div class="grid gap-5 lg:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)]">
        <section class="min-w-0">
          <div class="mb-3 flex items-center justify-between gap-3">
            ...
            >
              <Icon name="refresh" size="sm" :class="compositeRoutesLoading ? 'animate-spin' : ''" />
            </button>
          </div>
                  ...
                  >
                    <td class="max-w-[15rem] px-3 py-2">
                      ...
                      <div class="mt-1 flex flex-wrap items-center gap-1.5">
                        <span class="badge badge-gray">{{ compositeRouteMatchLabel(route.match_type) }}</span>
          ...
          <form class="space-y-3" @submit.prevent="saveCompositeRoute">
            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                <input v-model.number="compositeRouteForm.priority" type="number" min="1" step="1" class="input" />
              </div>
            </div>
            ...
            <div>
              ...
              <textarea v-model.trim="compositeRouteForm.notes" rows="2" class="input"></textarea>
            </div>
            ...
            <div class="flex items-center justify-between gap-3">
              ...
              <button type="submit" class="btn btn-primary" :disabled="compositeRouteSaving">
                <Icon v-if="!compositeRouteSaving" name="check" size="sm" class="mr-2" />
              ...
              >
                <div class="mb-2 flex items-center gap-2">
                  <span
                    :class="[
                      'badge',
                      compositePreviewDecision.matched ? 'badge-success' : 'badge-danger',
                    ]"
                  >
                    {{
                ...
                >
                  <div>
                    ...
                    {{ formatCompositePlatform(compositePreviewDecision.target_platform) }}
                  </div>
                  <div class="break-all">
      ...
      <template #footer>
        <div class="flex justify-end pt-4">
          <button type="button" class="btn btn-secondary" @click="closeCompositeRoutesModal">
            {{ t("common.close") }}
          </button>
        </div>
      </template>
...
const copyAccountsGroupLabel = (group: AdminGroup) => {
  ...
  return `${group.name} - ${platform} (${t("admin.groups.accountsCount", {
    count: group.account_count || 0,
  })})`;
};

// 复制账号的源分组选项（创建时）- Composite 可汇总任意具体平台账号
...
// 复制账号的源分组选项（编辑时）- Composite 可汇总任意具体平台账号，排除自身
...
const subscriptionGroupCount = computed(() =>
...
const createModelAllowlistSelectedCount = computed(
  () => selectedModelAllowlistCount(createModelAllowlistState),
);
const editModelAllowlistSelectedCount = computed(
  () => selectedModelAllowlistCount(editModelAllowlistState),
);
...
const submitCreateAllowlistCustomEntry = () => {
  ...
  if (error === null) {
    ...
    const errorKey = error === "invalid_wildcard" ? "invalidWildcard" : error;
    createAllowlistCustomErrorKey.value = `admin.groups.modelAllowlist.errors.${errorKey}`;
  }
};
const submitEditAllowlistCustomEntry = () => {
  ...
  if (error === null) {
    ...
    const errorKey = error === "invalid_wildcard" ? "invalidWildcard" : error;
    editAllowlistCustomErrorKey.value = `admin.groups.modelAllowlist.errors.${errorKey}`;
  }
};
...
const resetUnsupportedImagePricing = (form: ImagePricingFormState) => {
  if (supportsImagePricingPlatform(form.platform)) {
    resetDisabledBatchImagePricing(form);
    return;
  }
  form.allow_image_generation = false;
  form.allow_batch_image_generation = false;
  form.image_rate_independent = false;
  form.image_rate_multiplier = 1;
  form.batch_image_discount_multiplier = 0.5;
  form.batch_image_hold_multiplier = 0.6;
  form.image_price_1k = null;
  form.image_price_2k = null;
  form.image_price_4k = null;
};

const resetUnsupportedVideoPricing = (form: VideoPricingFormState) => {
  if (supportsVideoPricingPlatform(form.platform)) {
    return;
  }
...
const openCreateModal = () => {
  ...
  void loadModelAllowlistCandidates("create", 0, createForm.platform);
};
...
const handleCreateGroup = async () => {
  ...
  try {
    ...
    resetUnsupportedImagePricing(requestData);
    ...
    resetUnsupportedVideoPricing(requestData);
...
const handleEdit = async (group: AdminGroup) => {
  ...
  resetUnsupportedImagePricing(editForm);
  resetUnsupportedVideoPricing(editForm);
  ...
  for (const id of editCodexManifestConfig.value.account_ids) {
    adminAPI.accounts.getById(id).then((account) => {
      if (editingGroup.value?.id !== group.id) return;
      editCodexManifestAccountNames.value = { ...editCodexManifestAccountNames.value, [id]: account.name };
    }).catch(() => {
      // 已删除或不可读的账号保留 ID，供管理员移除。
    });
  }
  // 加载模型路由规则（异步加载账号名称）
  ...
  void loadModelAllowlistCandidates("edit", group.id, group.platform);
...
const handleUpdateGroup = async () => {
  ...
  if (editForm.platform === "openai" && editCodexManifestRef.value && !editCodexManifestRef.value.validate()) return;
  ...
  try {
    ...
    resetUnsupportedImagePricing(payload);
...
const isCurrentCompositeRoutesGroup = (groupID: number) =>
  showCompositeRoutesModal.value && compositeRoutesGroup.value?.id === groupID;

const invalidateCompositePreview = () => {
...
const loadCompositeRoutes = async () => {
  ...
  try {
    ...
    if (
      ...
      !isCurrentCompositeRoutesGroup(groupID)
    ) {
      return;
    }
    ...
    if (requestID === compositeRoutesRequestID) {
...
const saveCompositeRoute = async () => {
  ...
  try {
    ...
    if (
      !isCurrentCompositeRoutesGroup(groupID) ||
      compositeRouteEditingId.value !== editingID
    ) {
      return;
    }
    invalidateCompositePreview();
    ...
    if (!isCurrentCompositeRoutesGroup(groupID)) return;
    appStore.showError(
...
const deleteCompositeRoute = async (route: CompositeModelRoute) => {
  ...
  try {
    ...
    if (!isCurrentCompositeRoutesGroup(groupID)) return;
    if (compositeRouteEditingId.value === route.id) resetCompositeRouteForm();
    invalidateCompositePreview();
...
const previewCompositeRoute = async () => {
  const groupID = compositeRoutesGroup.value?.id;
  const model = compositePreviewModel.value.trim();
  const endpoint = compositePreviewEndpoint.value;
  if (!groupID || !model) return;
  const requestID = ++compositePreviewRequestID;
  compositePreviewLoading.value = true;
  compositePreviewDecision.value = null;
  try {
    const decision = await adminAPI.groups.previewCompositeRoute(groupID, { model, endpoint });
    if (
      requestID !== compositePreviewRequestID ||
      !isCurrentCompositeRoutesGroup(groupID) ||
      compositePreviewModel.value.trim() !== model ||
      compositePreviewEndpoint.value !== endpoint
    ) {
      return;
    }
...
watch(
  ...
  (newVal) => {
    ...
    resetUnsupportedImagePricing(createForm);
    resetUnsupportedVideoPricing(createForm);
    resetModelAllowlistState(createModelAllowlistState);
    void loadModelAllowlistCandidates("create", 0, newVal);
  },
);
...
watch(
  ...
  (newVal) => {
    ...
    if (editingGroup.value) {
      ...
      void loadModelAllowlistCandidates("edit", editingGroup.value.id, newVal);
    }
  },
);
