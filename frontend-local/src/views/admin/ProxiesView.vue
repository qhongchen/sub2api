          ...
          <div class="flex flex-col gap-4 p-5 lg:flex-row lg:items-start lg:justify-between">
            <div class="min-w-0 space-y-2">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span class="rounded-full border border-gray-200 bg-white px-2.5 py-1 text-xs font-medium text-gray-500 dark:border-dark-700 dark:bg-dark-800 dark:text-dark-300">
                  {{ proxies.length }} {{ t('common.visible') }}
                </span>
                <span class="rounded-full border border-primary-200 bg-primary-50 px-2.5 py-1 text-xs font-medium text-primary-700 dark:border-primary-500/20 dark:bg-primary-500/10 dark:text-primary-200">
                  {{ activeProxyCount }} {{ t('admin.accounts.status.active') }}
                </span>
                <span
        ...
        >
          ...
          <template #cell-status="{ value }">
            <span :class="['badge', value === 'active' ? 'badge-success' : 'badge-danger']">
              {{ value === 'expired' ? t('admin.proxies.expired') : t('admin.accounts.status.' + value) }}
            </span>
          </template>
    ...
    >
      ...
      >
        ...
        <div>
          ...
          <Select
            v-model="createForm.fallback_mode"
            :options="[
    ...
    >
      ...
      >
        ...
        <div>
          <label class="input-label">{{ t('admin.proxies.fallbackMode') }}</label>
          <Select
            v-model="editForm.fallback_mode"
            :options="[
              { label: t('admin.proxies.fallbackNone'), value: 'none' },
              { label: t('admin.proxies.fallbackProxy'), value: 'proxy' },
              { label: t('admin.proxies.fallbackDirect'), value: 'direct' }
...
const activeProxyCount = computed(() => proxies.value.filter(proxy => proxy.status === 'active').length)
...
const loadBackupProxyOptions = async () => {
  try {
    ...
    console.error('Failed to load backup proxy options:', error)
  }
}
const backupProxyOptions = (excludeId?: number) =>
  allProxiesForBackup.value
    .filter((proxy) => proxy.id !== excludeId)
    .map((proxy) => ({ label: `${proxy.name} (${proxy.host}:${proxy.port})`, value: proxy.id }))
...
const parseProxyUrl = (
  ...
  const regex = /^(https?|socks5h?):\/\/(?:([^:@\[\]]+):([^@\[\]]+)@)?(\[[0-9a-f:.]+\]|[^:\[\]]+):(\d+)$/i
