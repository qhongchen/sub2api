<template>
  <AppLayout>
    <div class="mx-auto w-full max-w-7xl space-y-6">
      <!-- Loading State -->
      <div v-if="loading" class="cch-panel-card flex items-center justify-center py-16">
        <div
      ...
      <!-- Settings Form -->
      <form v-else @submit.prevent="saveSettings" class="cch-settings-form" novalidate>
        <div class="cch-settings-page-header">
          <div class="flex min-w-0 items-start gap-4">
            <div class="cch-settings-page-icon">
              <Icon name="cog" size="lg" />
            </div>
            <div class="min-w-0">
              <h1 class="text-xl font-bold tracking-tight text-gray-950 dark:text-white md:text-2xl">
          ...
          <div v-show="activeTab !== 'backup'" class="cch-settings-header-actions">
            <button
              type="submit"
        ...
        <section class="cch-settings-content">
        ...
        <div v-show="activeTab === 'security'" class="space-y-6">
          ...
          <div class="card">
            ...
            <div class="space-y-5 p-6">
              ...
              <div v-if="form.api_key_acl_trust_forwarded_ip">
                <label
              ...
              <div class="border-t border-gray-100 pt-4 dark:border-dark-700">
                ...
                <div class="overflow-x-auto">
                  <table class="min-w-full text-sm">
                    ...
                    <tbody>
                      <tr
                        v-for="platform in PLATFORM_QUOTA_TYPES"
                        :key="platform"
                        class="align-top"
                      >
                        <td class="py-1 pr-4">
                        ...
                        <td class="py-1 pr-4">
                          <input
                            v-model.number="form.default_platform_quotas[platform]!.daily"
                            type="number"
                            step="0.01"
                            min="0"
                        ...
                        <td class="py-1 pr-4">
                          <input
                            v-model.number="form.default_platform_quotas[platform]!.weekly"
                            type="number"
                            step="0.01"
                            min="0"
                            class="input h-8 w-28 text-sm"
                        ...
                        <td class="py-1">
                          <input
                            v-model.number="form.default_platform_quotas[platform]!.monthly"
                            type="number"
                            step="0.01"
                            min="0"
                            class="input h-8 w-28 text-sm"
                        ...
                        <table class="min-w-full text-sm">
                          ...
                          <tbody>
                            ...
                            >
                              ...
                              <td class="py-1 pr-4">
                                <input
                                  v-model.number="authSourceDefaults[authSource.source].platform_quotas[platform]!.daily"
                                  type="number"
                                  step="0.01"
                                  min="0"
                                  class="input h-8 w-28 text-sm"
                              ...
                              <td class="py-1 pr-4">
                                <input
                                  v-model.number="authSourceDefaults[authSource.source].platform_quotas[platform]!.weekly"
                                  type="number"
                                  step="0.01"
                                  min="0"
                                  class="input h-8 w-28 text-sm"
                              ...
                              <td class="py-1">
                                <input
                                  v-model.number="authSourceDefaults[authSource.source].platform_quotas[platform]!.monthly"
                                  type="number"
                                  step="0.01"
                                  min="0"
                                  class="input h-8 w-28 text-sm"
            ...
            <div v-if="form.channel_monitor_enabled" class="space-y-5">
              ...
              <div v-if="form.channel_monitor_mode === 'v2'" class="space-y-4">
                ...
                <div class="flex items-start justify-between gap-4">
                  ...
                  <Toggle
                    v-model="form.channel_monitor_hide_user_ranking"
                    :aria-label="t('admin.settings.features.channelMonitor.hideUserRanking')"
                  />
                </div>
              </div>
        ...
        <!-- Save Button -->
        <div v-show="activeTab !== 'backup'" class="cch-settings-save-bar">
          <button
            type="submit"
            :disabled="saving || loadFailed"
...
const PLATFORM_QUOTA_TYPES: PlatformType[] = [
  "anthropic",
  "openai",
  "gemini",
  "antigravity",
...
type ForwardedClientIpHeaderResult = 'added' | 'invalid' | 'duplicate' | 'full'

function normalizeForwardedClientIpHeader(raw: string): string {
  return raw.trim().replace(/\s+/g, '-')
}

function isValidForwardedClientIpHeader(header: string): boolean {
  return /^[!#$%&'*+.^_`|~0-9A-Za-z-]+$/.test(header)
}
...
function normalizeForwardedClientIpHeaders(value: unknown): string[] {
  ...
  for (const raw of value) {
    ...
    if (
      !header ||
      !isValidForwardedClientIpHeader(header) ||
      seen.has(key) ||
...
function addForwardedClientIpHeader(raw: string): ForwardedClientIpHeaderResult {
  const header = normalizeForwardedClientIpHeader(raw)
  if (!header || !isValidForwardedClientIpHeader(header)) {
    return 'invalid'
  }
  if (
...
function handleForwardedClientIpHeaderPaste(event: ClipboardEvent) {
  ...
  for (const part of text.split(/[\s,;]+/)) {
    ...
    const result = addForwardedClientIpHeader(part)
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      forwarded_client_ip_headers: normalizeForwardedClientIpHeaders(form.forwarded_client_ip_headers),
...
.cch-settings-page-header {
  @apply flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between;
}

.cch-settings-page-icon {
  @apply hidden h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-primary-200/70 bg-primary-50 text-primary-600 sm:flex;
  @apply dark:border-primary-400/20 dark:bg-primary-500/10 dark:text-primary-300;
}

.cch-settings-header-actions {
  @apply flex shrink-0 flex-wrap items-center gap-2;
}

.cch-settings-layout {
  @apply space-y-5 lg:grid lg:grid-cols-[220px_minmax(0,1fr)] lg:gap-6 lg:space-y-0;
}

.cch-settings-content {
  @apply min-w-0 space-y-6;
}

.cch-settings-content :deep(.card) {
  @apply rounded-2xl border border-gray-200/60 bg-white/70 shadow-sm backdrop-blur-lg transition-colors duration-200;
  @apply dark:border-white/[0.08] dark:bg-dark-900/50;
}

.cch-settings-content :deep(.card > div:first-child) {
  @apply border-gray-100/80 bg-white/30 dark:border-white/[0.08] dark:bg-white/[0.02];
}

.cch-settings-save-bar {
  @apply flex justify-end rounded-2xl border border-gray-200/60 bg-white/70 p-4 shadow-sm backdrop-blur-lg;
  @apply dark:border-white/[0.08] dark:bg-dark-900/50;
}

.settings-tabs-shell {
  @apply sticky z-20 rounded-2xl border border-gray-200/60 bg-white/70 p-1.5 shadow-sm backdrop-blur-lg;
  @apply dark:border-white/[0.08] dark:bg-dark-900/60;
  top: 4.75rem;
}
...
.settings-tab {
  @apply relative isolate flex h-10 min-w-[6.75rem] shrink-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-xl border border-transparent px-3 text-sm font-medium text-gray-500 outline-none transition-colors duration-200 ease-out dark:text-gray-400;
}
...
@media (min-width: 1024px) {
  ...
  .settings-tab {
    @apply h-auto min-h-[2.75rem] w-full min-w-0 justify-start px-3 py-2.5 text-left;
  }
...
.settings-tab-active {
  @apply border-primary-200/80 bg-primary-50 text-primary-700 shadow-sm dark:border-primary-400/30 dark:bg-primary-500/10 dark:text-primary-200;
}
