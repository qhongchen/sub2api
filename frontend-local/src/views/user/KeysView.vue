    ...
    <TablePageLayout>
      <template #actions>
        <section class="cch-panel-card overflow-hidden">
            ...
            <div class="min-w-0 space-y-2">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span class="rounded-full border border-gray-200 bg-white px-2.5 py-1 text-xs font-medium text-gray-500 dark:border-dark-700 dark:bg-dark-800 dark:text-dark-300">
                  {{ pagination.total }} {{ t('nav.apiKeys') }}
                </span>
                <span class="rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-xs font-medium text-emerald-700 dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-200">
                  {{ keyOverview.active }} {{ t('common.active') }}
                </span>
              </div>
          ...
          <div class="grid border-t border-gray-200/70 dark:border-white/[0.08] sm:grid-cols-2 lg:grid-cols-4">
            <div class="key-overview-cell">
              <span>{{ t('common.total') }}</span>
              <strong>{{ pagination.total.toLocaleString() }}</strong>
              <small>{{ t('keys.apiKey') }}</small>
            </div>
            <div class="key-overview-cell">
              <span>{{ t('common.active') }}</span>
              <strong>{{ keyOverview.active.toLocaleString() }}</strong>
              <small>{{ keyOverview.visible }} {{ t('common.total') }}</small>
            </div>
            <div class="key-overview-cell">
              <span>{{ t('keys.quota') }}</span>
              <strong>{{ keyOverview.withQuota.toLocaleString() }}</strong>
              <small>${{ formatMoney(keyOverview.quotaUsed) }} / ${{ formatMoney(keyOverview.quotaLimit) }}</small>
            </div>
            <div class="key-overview-cell">
              <span>{{ t('keys.usage') }}</span>
              <strong>${{ formatMoney(keyOverview.totalActualCost) }}</strong>
      ...
      <template #filters>
        <div class="cch-toolbar-card flex flex-col gap-3">
          <div class="flex flex-wrap items-center gap-3">
            ...
            <div class="ml-auto text-xs text-gray-500 dark:text-dark-400">
              {{ keyOverview.visible }} / {{ pagination.total }} {{ t('nav.apiKeys') }}
            </div>
          </div>
          <EndpointPopover
...
const keyOverview = computed(() => {
  const visibleKeys = apiKeys.value
  return {
    visible: visibleKeys.length,
    active: visibleKeys.filter((key) => key.status === 'active').length,
    withQuota: visibleKeys.filter((key) => key.quota > 0).length,
    quotaLimit: visibleKeys.reduce((sum, key) => sum + (key.quota || 0), 0),
    quotaUsed: visibleKeys.reduce((sum, key) => sum + (key.quota_used || 0), 0),
    totalActualCost: visibleKeys.reduce(
      (sum, key) => sum + (usageStats.value[key.id]?.total_actual_cost ?? 0),
      0
    )
  }
})
...
<style scoped>
.key-overview-cell {
  @apply flex min-h-[112px] flex-col justify-between gap-3 border-b border-gray-200/70 p-4;
  @apply dark:border-white/[0.08];
}

.key-overview-cell span {
  @apply text-sm font-medium text-gray-500 dark:text-dark-300;
}

.key-overview-cell strong {
  @apply text-2xl font-semibold tracking-tight text-gray-950 dark:text-white;
}

.key-overview-cell small {
  @apply text-xs text-gray-500 dark:text-dark-400;
}

@media (min-width: 640px) and (max-width: 1023px) {
  .key-overview-cell:nth-child(odd) {
    @apply border-r;
  }

  .key-overview-cell:nth-last-child(-n + 2) {
    @apply border-b-0;
  }
}
...
@media (min-width: 1024px) {
  ...
  .key-overview-cell:last-child {
    @apply border-r-0;
  }
}

.cch-key-table :deep(.table-wrapper) {
  @apply bg-transparent;
}

.cch-key-table :deep(table) {
  border-collapse: separate;
  border-spacing: 0;
}

.cch-key-table :deep(thead) {
  @apply bg-gray-50/70 dark:bg-white/[0.03];
}

.cch-key-table :deep(th) {
  @apply border-b border-gray-200/70 text-[11px] font-semibold uppercase tracking-normal text-gray-500;
  @apply dark:border-white/[0.08] dark:text-dark-300;
}

.cch-key-table :deep(tbody) {
  @apply bg-transparent;
}

.cch-key-table :deep(tbody tr) {
  @apply transition-colors hover:bg-gray-50/70 dark:hover:bg-white/[0.03];
}

.cch-key-table :deep(td) {
  @apply align-middle border-b border-gray-100/80 dark:border-white/[0.06];
}

.cch-key-table :deep(code.code) {
  @apply rounded-full border border-gray-200 bg-gray-50 px-2 py-1 text-[11px] text-gray-700;
  @apply dark:border-dark-700 dark:bg-dark-800 dark:text-dark-200;
}

.cch-key-table :deep(> .space-y-3 > div) {
  @apply rounded-2xl border-gray-200/70 bg-white/70 shadow-sm backdrop-blur-lg dark:border-white/[0.08] dark:bg-dark-900/50;
}
</style>
