...
func usageRecordContext(parent context.Context, base context.Context) context.Context {
	...
	if startedAt, ok := parent.Value(ctxkey.RequestStartedAt).(time.Time); ok && !startedAt.IsZero() {
		base = context.WithValue(base, ctxkey.RequestStartedAt, startedAt)
	}
	return base
}
...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	sessionHashBody := requestRecordClientBody(c, body)
	...
	for {
		...
		// Compact 心跳不构成语义响应，不能阻止后续 failover 换号。
		writerSizeBeforeForward := service.OpenAICompactKeepaliveAdjustedWrittenSize(c)
		sessionIdentity := requestrecord.SessionResolver{}.ResolveOpenAI(c.Request, sessionHashBody)
		...
		h.recordCyberPolicyIfMarked(c, apiKey, account, subscription, reqModel, err != nil, cyberBlockKeyHTTP, clientRequestedUsageFields(c, channelMapping, reqModel, ""), service.HashUsageRequestPayload(sessionHashBody))
		...
		requestPayloadHash := service.HashUsageRequestPayload(sessionHashBody)
		inboundEndpoint := GetInboundEndpoint(c)
		completeRequestRecord(c, h.requestRecordService, &requestrecord.CompleteInput{
			RequestID:        recordRequestID(recordHandle, c),
...
func (h *OpenAIGatewayHandler) Messages(c *gin.Context) {
	...
	for {
		...
		if upstreamModel == "" && channelMappingMsg.Mapped {
			upstreamModel = channelMappingMsg.MappedModel
		}
